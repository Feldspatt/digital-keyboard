...
export function createOscillator(frequency: number, type: OscillatorType = 'sine') {
...
export function stopOscillator(osc: OscillatorNode) {
    ...
    if(!localGain) {
        throw new Error(`Unable to stop Oscillator: ${osc}, related local gain not found!`)
    }
...
function rampMasterGain(){
    const nextGain = activeOscillators.length > 0 ? baseGain / activeOscillators.length : 0
...
`)
    ...
    masterGain.gain.setValueAtTime(masterGain.gain.value, audioContext.currentTime); // Set current gain
    masterGain.gain.linearRampToValueAtTime(nextGain, audioContext.currentTime + 0.1);
}